 ...
 * the backend (env vars, the engine's exported function, the legacy app's
 * storage key, a document's filename) and are renamed there, not here.
 */
const SERVER_OWNED = /HUNTER_[A-Z_]+|hunter_state|hunterScore|HUNTER-PROJECT-OVERVIEW/g;

const locales = import.meta.glob<string>("./**/*.json", { query: "?raw", import: "default", eager: true });
const sources = import.meta.glob<string>(["./**/*.ts", "./**/*.tsx", "!./brand.test.ts"], { query: "?raw", import: "default", eager: true });
const indexHtml = import.meta.glob<string>("../index.html", { query: "?raw", import: "default", eager: true });
const served = import.meta.glob<string>("../public/*.{svg,html,json,txt,webmanifest}", { query: "?raw", import: "default", eager: true });
...
describe("brand", () => {
  ...
  it("actually catches the old name (so a green run means something)", () => {
    ...
    expect(offenders({ "x.ts": "const key = 'HUNTER_TODAY'; hunterScore(); // hunter_state" })).toEqual([]);
  });
});
