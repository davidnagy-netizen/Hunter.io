...
The catalog is now built from live sources (see [README.md](README.md) §2); the
nine hand-authored Hungarian calls below remain in it as curated reference
entries, each carrying full hard-rule and soft-rule definitions, funding bands,
support intensity, deadline, required documents and an official source reference:
...
> **Status note.** Sections 1–7 above describe the original front-end prototype.
> The system has since been built out against live data; see
> [README.md](README.md) for the current architecture. This section records what
> is working now and what is still open.
...
- The ask-and-recalculate loop for unknown fields — answers now apply company-wide,
  so one answer resolves every call that would have asked the same question
...
- **Live data connectors** — 600+ real calls from the EU Funding & Tenders Portal
  (SEDIA), refreshed by `npm run refresh`, plus the nine curated Hungarian entries
- **Full-text search** — BM25 over the whole catalog with facets, filters and paging
- **Consortium modelling** — the requirement most Hungarian SMEs are excluded by,
  with an estimated partner share instead of the whole-consortium grant
- **Persistence** — profile, answers and favourites survive a reload
- **Benchmarks** — 400 funded Hungarian projects from Kohesio, as comparables

### Not yet implemented

| Missing capability | Note |
|---|---|
| **Hungarian national connector** | `palyazat.gov.hu` and `kap.gov.hu` were unavailable during the build; the nine curated entries stand in, and flow through the same pipeline a real connector would |
| **Accounts and billing** | Deliberately out of scope — no registration, authentication, trial enforcement or payment |
| **Notifications** | Smart alerts at 30/14/7/3 days, email delivery, `.ics` calendar export |
| **Hunter Plus** | The AI application-writing workspace — entry point and upsell flow are visible, drafting is still demo content |
