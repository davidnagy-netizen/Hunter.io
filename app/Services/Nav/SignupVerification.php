...
final class SignupVerification
{
    ...
    {
        $token = $request->cookie('fundor_signup');
    ...
    {
        try {
            $data = json_decode(Crypt::decryptString((string) $request->input('verification_receipt')), true, flags: JSON_THROW_ON_ERROR);
        } catch (\Throwable) {
            throw new ApiError('VERIFICATION_EXPIRED', 422);
        }
        $hash = hash('sha256', (string) $request->cookie('fundor_signup'));
