...
import { deadlineTone } from "../domain/calendar";
import "../i18n";
...
export function DeadlineRow({ item }: { item: RankedOpportunity }) {
  const { t } = useTranslation("opportunities");
  const { date } = useFormat();
  const days = item.res.elig.days;

  return (
    <li>
      <Link
        to={`/app/opportunities/${encodeURIComponent(item.opp.id)}`}
      ...
      >
        ...
        <span className="min-w-0 flex-1 truncate">
          {item.opp.program} — {item.opp.title}
        </span>
        <span className="shrink-0 text-muted">
          {date(item.opp.deadline)}
          {days <= 14 ? <b className="text-amber"> · {t("calendar.days", { n: days })}</b> : null}
        </span>
      </Link>
    </li>
  );
